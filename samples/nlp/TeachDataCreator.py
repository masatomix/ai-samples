...
from nlp.MecabFacade import MecabFacade
import configparser
import hashlib
import os
...
class TeachDataCreator(object):
    ...
    def save_teach_data(self, morphs, dir_name, file_name=None):

        os.makedirs(dir_name, exist_ok=True)

        if file_name is None:
            path = os.path.join(dir_name, hashlib.md5(repr(morphs).encode('utf-8')).hexdigest() + ".txt")
        else:
    ...
    def concat_teach_data(self, dir):
        ...
        save_path = os.path.join(dir, "result.txt")
